...
export default class Board {
    constructor() {
        ...
        this.currentRound = 1;
        this.matchValidationTime = 0; // Timer for validating matches
        this.pendingMatch = false; // Whether we have a pending match to validate
        this.totalRounds = 3;
        this.baseRounds = 3; // Base number of rounds
        this.extraRounds = 0; // Extra rounds earned through scoring
        this.roundScores = []; // Track score for each round
        this.lastExtraRoundThreshold = 0; // Track the last score threshold where we awarded an extra round
    ...
    findAvailableColumn(excludeNearTop = true) {
    ...
    update(dt, timestamp) {
        ...
        this.scorePopups = this.scorePopups.filter(popup => Date.now() - popup.startTime < popup.duration);
        // Move shoes first
    ...
    renderExplosion(ctx, explosionTime) {
        ...
        if (flashOpacity > 0) {
            ...
            ctx.fillRect(this.playerCol * game.block, this.playerRow * game.block, game.block * 2, // Just cover the player width
        ...
        if (game.pict) {
            ...
            ctx.drawImage(game.pict, sx, sy, 44, 44, // Source coordinates (sprite sheet)
    ...
    showHighScorePopup(wasFirstTime) {
