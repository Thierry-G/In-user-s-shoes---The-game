...
export class UIEffects {
     ...
     */
    static drawExplosion(ctx, img, playerCol, playerRow, currentTs, explosionStart) {
     ...
     */
    static showGameOver(ctx, canvasWidth, canvasHeight) {
        // Dim the screen
        ctx.save();
     ...
     */
    static drawTouchHint(ctx, playerCol, playerRow, currentTs, hintPulseStart) {
        const block = game.block;
     ...
     */
    static isPointInHintCircle(x, y, playerCol, playerRow) {
        const block = game.block;
        const totalRowsAbs = game.topReserved + game.rows + game.bottomReserved;
        const hintRow = Math.min(playerRow + 1, totalRowsAbs - 1);
        const halfBlock = game.block * 0.5;
     ...
     */
    static isPointInButton(x, y, button) {
        return (x >= button.x &&
            x <= button.x + button.w &&
            y >= button.y &&
            y <= button.y + button.h);
     ...
     */
    static drawShoeExplosion(ctx, img, col, row, currentTs, startTs) {
        const block = game.block;
...
//# sourceMappingURL=UIEffects.js.map