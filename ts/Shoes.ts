...
/**
 * Definition of a shoe sprite frame in the sprite sheet
 */
export interface ShoeFrame {
	side: ShoeSide;
	yIndex: number; // 0..22
	sx: number;     // source x in sprite (0 for left, 44 for right)
	sy: number;     // source y in sprite (yIndex * 44)
	sWidth: number; // 44
	sHeight: number;// 44
}
...
export default class Shoes {
	 ...
	 */
	getFrame(side: ShoeSide, yIndex: number): ShoeFrame {
	 ...
	 */
	drawAtGrid(side: ShoeSide, yIndex: number, col: number, row: number, yOffset: number = 0) {
		...
		try {
			...
			console.error("Error drawing shoe:", err, 
				"sx:", frame.sx, 
				"sy:", frame.sy, 
				"sw:", frame.sWidth, 
				"sh:", frame.sHeight,
	 ...
	 */
	drawAtGridSpriteX(spriteX: number, yIndex: number, col: number, row: number, highlightLevel: number = 0) {
		if (!this.image || !game.ctx) {
			console.log("Cannot draw shoe - image or context not available");
			return;
		}
		...
		try {
			...
			game.ctx.strokeRect(
				Math.floor(dx) + 0.5, 
				Math.floor(dy) + 0.5, 
				Math.floor(game.block) - 1, 
				Math.floor(game.block) - 1
			);
		} catch (err) {
			console.error("Error drawing shoe with sprite X:", err, 
				"sx:", spriteX, 
				"sy:", sy, 
				"sw:", 44, 
				"sh:", 44,
