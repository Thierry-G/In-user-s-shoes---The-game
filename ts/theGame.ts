...
export class theGame {
    ...
    game!: Ui;
    pict: string | null = null;
    private isDragging = false;
    private playerCol = 0; // left-most grid column for player
    private playerRow = 0; // target row (within reserved rows)
    // Touch hint pulse state
    private hintPulseStart: number | null = null;
    private currentTs: number = 0;
    // Game Over UI: clickable button bounds
    private playAgainBtn: { x: number; y: number; w: number; h: number } | null = null;
    // Start Game button bounds
    private startGameBtn: { x: number; y: number; w: number; h: number } | null = null;
    // Game state
    private gameStarted: boolean = false;
    // Start screen state
    private showingStartScreen: boolean = true;
    // Button pulse animation
    private pulseAmount: number = 0;
    ...
    private roundExplosion: boolean = false; // Track if explosion is for round end vs game over
    private hidePlayerShoes: boolean = false; // Flag to temporarily hide player shoes after explosion
    private lastTs = 0;
    private rafId: number | null = null;
    // Round start waiting state
    private waitingForRoundStart: boolean = false;
    private roundStartMessage: string = '';
    private roundStartTime: number = 0;
    private ignoreInputUntil: number = 0; // Timestamp to ignore input until
    ...
    private drawTouchHint(ctx: CanvasRenderingContext2D) {
        ...
        if (this.hintPulseStart != null) {
            ...
            if (t >= 1) this.hintPulseStart = null;
        }
        ctx.restore();
    }
    ...
    private showStartScreen() {
        ...
        const img = game.pict as HTMLImageElement;
    ...
    private startMinimalLoop() {
        ...
        const startScreenLoop = (timestamp: number) => {
    ...
    private drawExplosion(ctx: CanvasRenderingContext2D) {
        const img = game.pict as HTMLImageElement;
        if (!img) {
        ...
        const start = this.explosionStart ?? now;
    ...
    private startGameOverLoop() {
        ...
        const gameOverLoop = (timestamp: number) => {
            // Update current time
            this.currentTs = timestamp;
    ...
    private setupInput() {
        const touchHandler = (ev: MouseEvent | TouchEvent) => {
    ...
    private setupKeyboardInput() {
        window.addEventListener('keydown', (ev: KeyboardEvent) => {
            if (this.showingStartScreen || board.isGameOver()) return;
    ...
    private updatePlayerPosition(ev: MouseEvent | TouchEvent) {
...
window.addEventListener('load', () => {
    ...
    (window as any).theGameInstance = gameInstance;
});
