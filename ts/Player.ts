...
// Define a global interface for accessing the board component
...
let boardInstance: BoardInterface | null = null;

export function setBoardInstance(board: BoardInterface) {
    boardInstance = board;
}
...
export default class Player {
     ...
     */
    renderPlayer(ctx: CanvasRenderingContext2D, col: number, row: number, hideShoes: boolean = false) {
        if (!ctx || !this.image || !boardInstance) return;
     ...
     */
    drawAtGrid(col: number, row: number, offset: number = 0) {
        if (!game.ctx) return;
